...
import { deleteShelter, getShelters, updateShelter } from '../../api/shelters.js'
import { DISTRICTS } from '../../constants/districts.js'
import { SHELTER_STATUSES } from '../../constants/enums.js'
import { occupancyColor, occupancyStatus } from '../../utils/occupancy.js'
...
export default function ShelterGrid({ refreshKey = 0 }) {
  ...
  async function removeShelter(shelter) {
    setUpdatingId(shelter._id)
    setUpdateError('')
    try {
      await deleteShelter(shelter._id)
      await load()
    } catch (err) {
      setUpdateError(
        err.response?.data?.message ||
          'Unable to delete this shelter. Please try again.',
      )
    } finally {
      setUpdatingId(null)
    }
      ...
      {loading ? (
        ...
        <div className="shelter-grid__list">
          ...
          {shelters.map((shelter) => {
            ...
            const color = occupancyColor(status)
            ...
            return (
              <article className="shelter-card" key={shelter._id}>
                <div className="shelter-card__top">
                  ...
                  <Chip className="shelter-card__chip" variant={status}>
                    {status}
                  </Chip>
                </div>
